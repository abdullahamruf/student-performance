...
Original file is located at
    https://colab.research.google.com/drive/1mr-w6koe4j45rr28Y5pHaZIwMyf1fXFH
"""
...
new_dataset = new_dataset.drop('hsc_result', axis=1)
new_dataset.head()
...
X = new_dataset.drop(['ssc_result'], axis=1)
...
# # Define a list of classifiers (base models)
# classifiers = [
#     RandomForestClassifier(),
#     GradientBoostingClassifier()
# ]

# # Create a list of tuples containing all possible combinations of scalers and classifiers
# models = []
# for scaler in scalers:
#     for classifier in classifiers:
#         models.append((str(scaler) + '-' + classifier.__class__.__name__, make_pipeline(scaler, classifier)))

# # Define the meta-model
# meta_model = RandomForestClassifier()

# # Create the stacking ensemble using all combinations of scalers and classifiers
# stacking_ensemble = StackingClassifier(estimators=models, final_estimator=meta_model)
...
# explainer = shap.KernelExplainer(stacking_ensemble.predict_proba, X_train)
...
def prob(data):
    return stacking_ensemble.predict_proba(data)

import lime
import lime.lime_tabular

explainer = lime.lime_tabular.LimeTabularExplainer(
    df[list(X.columns)].astype(float).values,
    mode='classification',
    training_labels=df['ssc_result'],
    feature_names=list(X.columns),

)

i = 2
exp = explainer.explain_instance(df.loc[i,list(X.columns)].astype(int).values, prob, num_features=5)
...
plt.title('ROC Curve for Multiclass Classification (SSC Result)')
